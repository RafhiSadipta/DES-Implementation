...
class BinString(str):
    ...
    def __xor__(self, other):
        assert isinstance(other, (str, BinString)), \
            'Undefined XOR (^) operation for type %s' % type(other)
        if isinstance(other, str):
            for char in other:
                if char not in ('0', '1'):
                    raise ValueError('Undefined XOR (^) operation for string %s' % other)
...
class DES(object):
    ...
    def _validate_length(self, string, length):
        assert len(string) == length, 'String %s length\'s not %s' % (string, length,)

    def _group_by(self, string, by):
        # Mengelompokkan string berdasarkan jumlah karakter 'by'
    ...
    def _permute_with(self, string, permutation):
        # Permutasi string sesuai dengan tabel permutasi
        return ''.join([string[i-1] for i in permutation])

    def f(self, r, k):
        # Fungsi f untuk cipher DES
    ...
    def is_hex_string(self, string):
        hex_chars = set("0123456789abcdefABCDEF")
        return all(char in hex_chars for char in string)

    def encrypt(self, string, **kwargs):
        # Cek jika hexstring diaktifkan dan validasi string
        if kwargs.get('hexstring', False):
            if not self.is_hex_string(string):
                return "Input mengandung karakter non-hexadecimal."
        if kwargs.get('key'):
            self._set_key(kwargs['key'], kwargs.get('hexkey', False))
        self._set_message(string, kwargs.get('hexstring', False))
        assert len(self.key) > 0, 'Tidak ada input kunci'
...
def main():
    des = DES()
    print("Selamat datang di Aplikasi DES!")
    ...
    while True:
        ...
        if choice == '1':
            # Enkripsi
            print("\nAnda memilih Enkripsi.")
            print("Apakah input Anda dalam format hexstring atau unicode?")
            input_format = input("Ketik 'hexstring' atau 'unicode': ").strip().lower()

            if input_format == 'hexstring':
                input_text = input("Masukkan plaintext (hexstring): ").strip()
                key = des.generate_des_key()
                print(f"Generated DES Key: {key}")
                cipher = des.encrypt(input_text, hexstring=True, key=key, hexkey=True, hexresult=True)
                print(f"Hasil Enkripsi: {cipher}")

            elif input_format == 'unicode':
                input_text = input("Masukkan plaintext (unicode): ").strip()
                key = des.generate_des_key()
                print(f"Generated DES Key: {key}")
                cipher = des.encrypt(input_text, key=key, hexkey=True, hexresult=True)
                print(f"Hasil Enkripsi: {cipher}")

            else:
                print("Format input tidak dikenali!")
